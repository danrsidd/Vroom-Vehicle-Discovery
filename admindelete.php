  ...
  <body>
    ...
    <section style="background: rgb(2,0,36); background: linear-gradient(90deg, rgba(2,0,36,1) 0%, rgba(255,226,145,0.8925945378151261) 0%);");>
      <div class="container">
        ...
        <br>
        <br>
                <span class="section-heading-upper">Vroom Admin</span>
                <span class="section-heading-lower">Delete Tool</span>         
        </h2>
...
<?php
		...
		$sql = "SELECT mdl.model_id, mk.make_name, mdl.model_name, mdl.model_year, mdl.model_value
				FROM models AS mdl INNER JOIN makes AS mk
				ON mdl.make_id = mk.make_id
				ORDER BY mdl.model_id ASC;";
		...
		if($result)
		{
			...
			echo "<table>";
      		echo "<tr><td>ID</td><td>Make</td><td>Model</td><td>Year</td><td>Value</td></tr>";
			while($row=mysqli_fetch_assoc($result))
			{
				foreach($row as $key=>$value) ${$key}=$value;

				echo "<tr><td>$model_id</td><td>$make_name</td><td>$model_name</td><td>$model_year</td><td>$model_value</td></tr>";
			}
...
<form method="POST" action="deleteProcess.php" class="form-horizontal">
<h3 class= "text-center text-black d-none d-lg-block">Delete by Model</h3><br>
...
?>
    ...
    <input type="submit" button id="button1id" name="button1id" value="Delete" class="btn btn-success"></button>
	</div>
</form> 
